...
Mac802_11Ext::Mac802_11Ext() :
	...
	bind("Rate_Mode_", &RateMode_);
	bind("Noise_floor_", &noise_floor_);
	bind("Fixed_rate_", &fixedRate_);
	...
	arf = NULL;
	if(USE_ARF)
	  enableDefaultARF(RateMode_,fixedRate_,(u_int32_t)index_);
...
void Mac802_11Ext::setSINR(double d, double d1) {
  if((ModulationScheme)arf->getModScheme(SINR) != (ModulationScheme)arf->getModScheme(d))
    //Lek 05/10/2013 fprintf(stderr,"%f node#%d SINR changed:%f->%f Mod_scheme:%d->%d \n",Scheduler::instance().clock(),(int)index_,SINR,d,(ModulationScheme)arf->getModScheme(SINR),(ModulationScheme)arf->getModScheme(d));
  SINR = d;
}
...
void Mac802_11Ext::transmit(Packet *p, TXConfirmCallback callback) {
	...
	if(mh->dh_fc.fc_type == MAC_Type_Data) {
	  ...
	  struct hdr_cmn *ch = HDR_CMN(p);
	  //Lek 05/10/2013 fprintf(stderr,"123456 %f %d %d %d 0\n",Scheduler::instance().clock(),(int)(index_),tcph->seqno(),modulation_table[ch->mod_scheme_].NDBPS);
	  //	  fprintf(stderr,"transmit uid %d\n",hdr_cmn::access(p)->uid_);
	  if((int)hdr_cmn::access(p)->next_hop_ >= 0)
	    hdr_cmn::access(p)->ts_arr_ = Scheduler::instance().clock();
	  //Lek 05/10/2013 fprintf(stderr,"-123456 %f %d %d %f %d\n",Scheduler::instance().clock(),(int)index_,hdr_cmn::access(p)->uid_,hdr_cmn::access(p)->ts_arr_,ch->size_);
	}
...
double Mac802_11Ext::txtime(double psz, int mod_scheme) {
	...
	//	fprintf(stderr,"%.5f: %dsending %d bytes with %d\n",Scheduler::instance().clock(),(int)index_,datalen,DBPS);
	...
	//	fprintf(stderr,"txtime %f DBPS %d\n",t,DBPS);
	return (t);
}
...
void TXC::prepareMPDU(Packet *p) {
	...
	if(mac_->arf) {
	  ch->mod_scheme_ = (ModulationScheme)(mac_->arf)->getModScheme(mac_->getSINR());
	  //Lek 05/10/2013	  fprintf(stderr,"123456 %f %d 1.12 %f 0\n",Scheduler::instance().clock(),(int)(mac_->index_),mac_->getSINR());
	  //	  fprintf(stderr,"Distance__ %f mode %d\n",(mac_->macmib_).getDistance(),(mac_->macmib_).getMode());
	}

	//	fprintf(stderr,"prepare MPDU with mod %d\n",ch->mod_scheme_);
...
void TXC::handleTCTStimeout() {
	if (txc_state_==TXC_wait_CTS) {
		...
		if (shortretrycounter >= mac_->macmib_.getShortRetryLimit()) {
			...
			if((mac_->arf) != NULL)
			  (mac_->arf)->DoReportFinalRtsFailed();
...
void TXC::handleTACKtimeout() {
	if (txc_state_==TXC_wait_ACK) {
		...
		if (*counter >= limit) {
		  //Lek 05/10/2013 fprintf(stderr,"ack timeout limit reached %d\n",*counter);
			if (mac_->MAC_DBG)
				mac_->log("TXC", "ACK timeout, limit reached");
			if((mac_->arf) != NULL)
			  (mac_->arf)->DoReportFinalDataFailed();
			...
			if (ch->xmit_failure_) {
			  ch->xmit_reason_ = XMIT_REASON_RTS;
			  ch->xmit_failure_(pDATA->copy(), ch->xmit_failure_data_);
			  //Lek 05/10/2013 fprintf(stderr,"ch xmit failure, higher lyer support\n");
			}
...
void TXC::handleACKIndication() {
	...
	if (txc_state_==TXC_wait_ACK ) {
		...
		//Lek
		if((mac_->arf) != NULL) {
		  (mac_->arf)->successfulTransmission();
		  (mac_->arf)->DoReportDataOk();
		}
	} else {
...
void RXC::handleMsgFromBelow(Packet *p) {
	...
	case MAC_Type_Data:
		...
		case MAC_Subtype_Data:
			...
			//Lek 02/25/2013:: SINR-based rate control
			mac_->setSINR(p->txinfo_.RxPr,mac_->noise_floor_);
			//Lek 05/10/2013			fprintf(stderr,"%f RX DATA with SINR %.2f\n",Scheduler::instance().clock(),mac_->getSINR());
			//Lek 05/10/2013			fprintf(stderr,"123456 %f %d 1.12 %f 0\n",Scheduler::instance().clock(),(int)(mac_->index_),mac_->getSINR());
			break;
		default:
