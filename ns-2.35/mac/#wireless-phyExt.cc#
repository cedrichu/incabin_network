...
void WirelessPhyExt::handle_RXtimeout() {
	...
	if (cmh->error()) {
	  ...
	  // Lek 02/25/2013
	  double t = powerMonitor->getPowerLevel();
	  if(t > power_RX) {
	    pkt_RX->txinfo_.RxPr = power_RX/(t-power_RX);
	  } else
	    pkt_RX->txinfo_.RxPr = power_RX/noise_floor_;
	  if(powerMonitor->getPowerLevel() > power_RX + noise_floor_ + 2*pow(10,-19.0)) {
	    fprintf(stderr,"sending up %e %e %e %e\n",power_RX,powerMonitor->getPowerLevel(),powerMonitor->getPowerLevel()-power_RX,noise_floor_);
	  }
	  //	  pkt_RX->txinfo_.RxPr = power_RX;
	  //pkt_RX->txinfo_.RXThresh = noise_floor_;
	  /*fprintf(stderr,"sending up pwrRX %e %.4f (%.4f) SINR %e\n",power_RX,10.0*log10(power_RX),10.0*log10(powerMonitor->getPowerLevel()),pkt_RX->txinfo_.RxPr);*/
